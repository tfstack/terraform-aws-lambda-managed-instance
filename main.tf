# Root composition: wires VPC + security group + lambda_managed_instance + lambda_managed_function.
# Reusable modules live under modules/ and can be called directly from any Terraform root.
...
module "lambda_managed_function" {
  ...
  iam_role_name_prefix  = var.name_prefix
  description           = "Basic LMI smoke-test (root)"
